        ...
        try:
            if create_folder:
                ...
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, ".fp-btn-mkdir"))).click()
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, ".fp-mkdir-dlg-text > input")))

                if folder_name != '':
                    self.driver.find_element(
                        By.CSS_SELECTOR, ".fp-mkdir-dlg-text > input").send_keys(folder_name)
                    self.driver.find_element(
                        By.CSS_SELECTOR, ".fp-dlg-butcreate").click()
                    wait.until(expected_conditions.visibility_of_element_located(
                        (By.CSS_SELECTOR, '.fp-file.fp-folder'))).click()
                else:
                    self.driver.find_element(
                        By.CSS_SELECTOR, ".fp-mkdir-dlg-text > input").clear()
                    create_btn = self.driver.find_element(
                        By.CSS_SELECTOR, ".fp-dlg-butcreate")
                    result = "Cannot create folder" if create_btn.value_of_css_property(
                        'opacity') == '0.65' else 'Create folder successful'
                    return result == expected

            if upload_type == "select":
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, ".fp-btn-add"))).click()
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.LINK_TEXT, "Upload a file"))).click()
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.NAME, "repo_upload_file")))

                self.driver.find_element(
                    By.NAME, "repo_upload_file").send_keys(file_path)
                self.driver.find_element(
                    By.XPATH, "//button[contains(.,'Upload this file')]").click()
                wait.until(expected_conditions.visibility_of_any_elements_located(
                    (By.CSS_SELECTOR, '.fp-file')))
                self.driver.find_element(By.NAME, 'submitbutton').click()
                toast_message = wait.until(expected_conditions.visibility_of_element_located(
                ...
                if "The maximum size you can upload is 100MB." in expected:
                    toast_message = wait.until(expected_conditions.visibility_of_element_located(
                        (By.ID, "fp-msg-labelledby"))).text
                    self.driver.find_element(
                        By.CSS_SELECTOR, '.file-picker.fp-msg.fp-msg-error > button').click()
            ...
            if create_folder:
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, '.fp-contextmenu'))).click()
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, '.filemanager.fp-select.fp-folder > form > div:nth-child(1) > button:nth-child(2)'))).click()
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, '.fp-dlg-butconfirm'))).click()
                self.driver.find_element(By.NAME, 'submitbutton').click()
            else:
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, '.fp-file'))).click()
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, '.filemanager.fp-select.fp-file > form > div:nth-child(1) > button:nth-child(2)')))
                self.driver.find_element(
                    By.CSS_SELECTOR, '.filemanager.fp-select.fp-file > form > div:nth-child(1) > button:nth-child(2)').click()
                wait.until(expected_conditions.visibility_of_element_located(
                    (By.CSS_SELECTOR, '.fp-dlg-butconfirm'))).click()
                self.driver.find_element(By.NAME, 'submitbutton').click()
